...
| **WikiSearchService** | Full-text wiki search via SQLite FTS5 (porter-stemmed) on wiki content. Returns FTS5-ranked matches + auto-generated index.md catalog for complete wiki structure map. Falls back to trigram fuzzy matching on zero results. Powers wiki search (N3), @-mentions (N7), related sections (N4), AI wiki_search tool (H10). | [tech-sourcing #11](../tech-sourcing.md#11-full-text-search) |
...
| Component | Description | Sourcing Ref |
|-----------|-------------|-------------|
| **ToolOrchestrator** | Implements the function-calling loop: AI requests tool → orchestrator validates → executes (or prompts user confirmation) → captures result → feeds back to AI. Manages 14 provider-agnostic tools with parallel execution (`Task.WhenAll`, max 10 concurrent). Tools: read_file, list_files, search_files, apply_diff, write_to_file (5 file ops replacing text_editor), bash, web_search, web_fetch, image_search, wiki_search, memory, skill_load, ask_user_input, present_files. | [tech-sourcing #38](../tech-sourcing.md#38-anthropic-tool-schema-adoption) |
| **BashToolExecutor** | Executes shell commands via `cmd.exe` (with Git Bash/WSL fallback) in per-chat workspace-isolated `%LOCALAPPDATA%/MySecondBrain/workspace/{chat-id}/`. Absolute paths outside workspace are blocked pre-execution, with one exception: paths under `%LOCALAPPDATA%/MySecondBrain/skills/` are allow-listed for read/execute when a skill is loaded, enabling skill script execution via absolute paths from `<skill_resources>`. Writes outside workspace require user confirmation. Wiki directory and skills directory are read-only. Workspace deleted on chat deletion. | [tech-sourcing #17](../tech-sourcing.md#17-terminal--shell-execution-tool-use-h2) |
| **File Operation Executors (5 tools)** | Provider-agnostic schemas replacing the former `text_editor` tool. `ReadFileToolExecutor`: path validation, approval gate, binary detection. `ListFilesToolExecutor`: directory listing with recursive option. `SearchFilesToolExecutor`: regex search with file_pattern glob. `ApplyDiffToolExecutor`: SEARCH/REPLACE block parser, byte-for-byte match. `WriteToFileToolExecutor`: create/overwrite with safety flag, auto-create dirs. Read tools use approval gate for out-of-workspace paths; write tools restricted to workspace+artifacts. | [tech-sourcing #18](../tech-sourcing.md#18-file-generation--editing-tool-use-h3-h4) |
...
| **API History Logger** | Every `ILLMProvider` call appends raw request/response to `_api_history.json` in per-chat workspace. JSON format matches vision spec (role/content/ts structure with tool_use/tool_result/reasoning blocks). Powers the API History Viewer (X). Cleanup on chat deletion. | [roadmap.md](../roadmap.md#feature-11--codebase-realignment-14-tool-surface--vision-alignment) |
...
*Architecture document — Batch 1 of planning/ directory. Updated 2026-06-25: 14-tool provider-agnostic surface, per-chat workspace/artifacts isolation, parallel tool execution, API history logging, file viewer tab system, System Info component. See also: [`tech-stack.md`](tech-stack.md), [`abstractions.md`](abstractions.md).*
