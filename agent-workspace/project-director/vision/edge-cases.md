...
- **Diagnostics — log file disk space management:** Log files grow unbounded if not managed. Serilog rolling file sink retains 30 days of logs by default. After 30 days, old log files are automatically deleted. If disk space becomes critically low (<500MB free), logging pauses with an in-memory buffer warning. The user can clear logs manually via Settings → Diagnostics → Clear Logs.
- **Diagnostics — log file rotation:** Serilog rolling file sink creates one file per day. At midnight (or on the first log event after midnight), a new file is created. The previous day's file is closed and retained for 30 days.
- **Diagnostics — logs folder inaccessible:** If `%LOCALAPPDATA%\MySecondBrain\logs\` cannot be created (permissions, disk error, path too long), logging falls back to a minimal in-memory circular buffer (last 1000 events). Settings → Diagnostics shows: "⚠️ Log directory is not accessible. Logging to memory buffer only (last 1000 events)." The "Open Logs Folder" button shows an error toast if the folder cannot be opened.
- **Diagnostics — performance impact of Verbose logging:** Verbose level logs significantly more data and may impact app responsiveness, especially during streaming AI responses (additional I/O per token). The Settings UI warns: "Verbose logging may impact performance." If Verbose is enabled, a subtle amber indicator (⚠️) appears in the status bar while the app is running at Verbose level. Switching to Information or Debug removes the indicator.
- **Diagnostics — API key redaction verification:** All 8 categories that might reference API keys (LLM API Calls, Tier 1, Tier 2, WebSocket) must enforce redaction via Serilog destructuring policy. The `ApiKey` type is registered with `Destructure.ByTransforming<ApiKey>(_ => "[REDACTED]")`. This is a security requirement — no log file should ever contain a raw API key. The Architect must ensure this is tested.
- **Diagnostics — Clear Logs with file locks:** If another process (e.g., text editor, log viewer) has a log file open, deletion may fail for that file. The Clear Logs operation deletes all files that it can and reports: "Cleared [N]/[M] log files. [K] files could not be deleted — they may be open in another program."
- **Diagnostics — all categories disabled:** If the user disables all 8 categories, no log events are written to disk. The app operates silently with zero logging overhead. The Clear Logs and Open Logs Folder buttons remain functional.
- **Diagnostics — startup/shutdown with logging enabled:** Category 7 (Startup & Shutdown) logs begin before the DI container is fully built — the logger itself must be available early. Serilog's bootstrap logger pattern handles this: a minimal logger is configured first (console + rolling file), then replaced with the fully configured logger once settings are loaded. If Category 7 is OFF during startup, no startup events are logged.
