...
### [x] Step 4: Settings UI — Profiles Section (Model Configurations & Personas)
...
- **Step 4 — Settings UI Profiles Abstractions:** ModelConfigurationDisplayItem and PersonaDisplayItem wrappers with Summary property. SettingsViewModel extended with IModelConfigurationRepository, IPersonaRepository dependencies. Collections: ModelConfigurations, Personas, AvailableModels, AvailableApiKeys. Commands: AddModelConfig, SaveModelConfig, DuplicateModelConfig, DeleteModelConfig, EditModelConfig, CancelModelConfigEdit, AddPersona, SavePersona, DeletePersona, EditPersona, CancelPersonaEdit, FetchModels. ContextOverflowStrategyOptions (SlidingWindow/HardStop/AutoSummarize) and ChatModeOptions (Standard/TextCompletion) for ComboBox bindings. Temperature slider 0.0-2.0 with clamping. Auto-fetch with CancellationTokenSource for race-free model loading.
